...
//wo browse
    ...
    public function geteditwo(Request $req){
            ...
            $data = DB::table('wo_mstr')
                ...
                loc_desc,astype_code,astype_desc,wo_new_type,wo_impact,wo_impact_desc,wo_action')
    ...
    public function reportingwo(Request $req){
        ...
        //     dd('aaa');
            if ($req->repairtype == 'manual'){
                ...
                /* simpan file A211025 */
                if(isset($req->fileauto)){
                    foreach($req->fileauto as $upload){
                        $dataTime = date('Ymd_His');
                        $filename = $req->c_wonbr.$upload;

                        $alamaturl = '../public/uploadwofinish/'.$filename;
                        file_put_contents($alamaturl, $upload);
                        ...
                        DB::table('acceptance_image')
                            ->insert([
                                ...
                                'file_wonumber' => $req->c_wonbr,
                                'file_name'     => $upload, //nama file asli
                                'file_url'      => $alamaturl, 
                                'uploaded_at'   => Carbon::now()->toDateTimeString(),
                            ]);
                    }
                ...
                if($req->has('repaircode2')){
                    ...
                    $arrayrc2 = array([
                        ...
                        // for($i3 = 0; $i3 < count($req->spm2); $i3++){  
                        //     $arraydets2 = array([
                        //         'wo_dets_nbr'       => $req->c_wonbr,
                        //         'wo_dets_rc'        => $req->repaircode2[0],
                        //         'wo_dets_sp'        => $req->spm2[$i3],
                        //         'wo_dets_sp_qty'    => $req->qty2[$i3],
                        //         'wo_dets_ins'       => $req->ins2[$i3],
                        //         'wo_dets_rep_hour'  => $req->rph2[$i3],
                        //         'wo_dets_standard'  => $req->std2[$i3],
                        //         'wo_dets_created_at'=> Carbon::now('ASIA/JAKARTA')->toDateTimeString(),
                        //     ]);
                        //         // dd($arraydettemp);
                        //         DB::table('wo_dets')->insert($arraydets2);
    ...
    public function reportingwoother(Request $req) {
        ...
        if(isset($req->fileother)){
            foreach($req->fileother as $upload){
                $dataTime = date('Ymd_His');
                $filename = $req->o_wonbr.$upload;
    ...
    public function statusreportingwo(Request $req){
        ...
        if($srnbr == null){
            ...
            if($req->switch2 =='approve'){
                ...
                ->where('wo_nbr',$wonumber)
                // ->update(['wo_status'=>'closed','wo_reject_reason'=>$req->uncompletenote]); --> A210927
                ->update(['wo_status'=>'incomplete','wo_reject_reason'=>$req->uncompletenote]);
                toast('Work Order '.$wonumber.' incomplete ', 'success');
    ...
    public function downloadwofinish($wo)
    {
        return ($wo);
    }
