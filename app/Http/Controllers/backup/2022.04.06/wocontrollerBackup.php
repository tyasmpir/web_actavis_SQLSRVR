...
//wo browse
    ...
    public function closewo(Request $req)
    {
        ...
        toast('Work Order Successfuly deleted!','success');
        return back();
    }
    ...
    public function geteditwo(Request $req){
            ...
            $data = DB::table('wo_mstr')
                ->selectRaw('wo_nbr,wo_repair_type,wo_repair_group,wo_repair_type,wo_status,asset_desc,wo_approval_note,wo_creator,wo_reject_reason,wo_priority,wo_dept,dept_desc,wo_note,wo_sr_nbr,wo_status,wo_asset,asset_desc,wo_schedule,wo_duedate,wo_engineer1 as woen1,wo_engineer2 as woen2, wo_engineer3 as woen3,wo_engineer4 as woen4,wo_engineer5 as woen5,u1.eng_desc as u11,u2.eng_desc as u22, u3.eng_desc as u33, u4.eng_desc as u44, u5.eng_desc as u55, wo_mstr.wo_failure_code1 as wofc1, wo_mstr.wo_failure_code2 as wofc2, wo_mstr.wo_failure_code3 as wofc3, fn1.fn_desc as fd1, fn2.fn_desc as fd2, fn3.fn_desc as fd3,r1.rep_desc as r11,r2.rep_desc as r22,r3.rep_desc as r33,r1.rep_code as rr11,r2.rep_code as rr22,r3.rep_code as rr33, wo_finish_date,wo_finish_time,wo_repair_hour')
                ...
                ->leftjoin('rep_mstr as r1','wo_mstr.wo_repair_code1','r1.rep_code')
                ->leftjoin('rep_mstr as r2','wo_mstr.wo_repair_code2','r2.rep_code')
                ->leftjoin('rep_mstr as r3','wo_mstr.wo_repair_code3','r3.rep_code')
                ->leftJoin('dept_mstr','wo_mstr.wo_dept','dept_mstr.dept_code')
                ->leftjoin('xxrepgroup_mstr','xxrepgroup_nbr','wo_mstr.wo_repair_group')
                ->where('wo_mstr.wo_nbr','=',$nowo)
                ->get();
                // dd($data);
    ...
    public function approvewo(Request $req){
        ...
        ->update(['wo_status'=>'open','wo_repair_code1' =>$rc1,'wo_repair_code2'=>$rc2,'wo_repair_code3'=>$rc3,'wo_repair_group'=>$exprg,'wo_repair_type'=>$exprt]);

        toast('Job approved successfuly', 'success');
        ...
        ->where('wo_nbr',$req->aprwonbr)
        ->update(['wo_status'=>'closed']);
        toast('Job has been rejected', 'success');
        return redirect()->route('womaint');
        }
    ...
    public function editjob(Request $req){
        ...
        if($statuswo =='open'){
            ...
            toast('Job started successfuly', 'success');
        ...
        toast('Job has been abandoned', 'success');
        return redirect()->route('wojoblist');
    ...
    public function reportingwo(Request $req){
        ...
        if($req->repairtype == 'code'){
            ...
            if($req->has('repaircode2')){
                ...
                $dborigin2 = DB::table('rep_mstr')
                ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
                ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
                ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
                ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
                ->where('rep_mstr.rep_code','=',$rc2)
            // ->groupBy('spt_code')
                ->orderBy('spt_desc')
                ->get();
                // dd($req->all());
            ...
            if($req->has('repaircode3')){
                ...
                $dborigin3 = DB::table('rep_mstr')
                ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
                ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
                ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
                ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
                ->where('rep_mstr.rep_code','=',$rc3)
                // ->groupBy('spt_code')
                ->orderBy('spt_desc')
                ->get();
    ...
    public function openprint(Request $req,$wo){
        ...
        $repair = DB::table('wo_mstr')
                ->selectRaw('r1.rep_desc as r11,r2.rep_desc as r22, r3.rep_desc as r33')
                ->leftjoin('rep_mstr as r1','wo_mstr.wo_repair_code1','r1.rep_code')
                ->leftjoin('rep_mstr as r2','wo_mstr.wo_repair_code2','r2.rep_code')
                ->leftjoin('rep_mstr as r3','wo_mstr.wo_repair_code3','r3.rep_code')
                ->where('wo_mstr.wo_nbr','=',$wo)
    ...
    public function openprint2(Request $req,$wo){
        ...
        if($statusrepair->wo_repair_type == 'group'){
            ...
            for($i = 0; $i<count($arrayrepaircode);$i++){
                ...
                // dd($i);
                $repair[$i] = DB::table('rep_mstr')
                                ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
                                ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
                                ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
                                ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
                                ->where('rep_mstr.rep_code','=',$arrayrepaircode[$i])
                                // ->groupBy('spt_code')
                                ->orderBy('repdet_step')
                                ->get();
        ...
        // dd($data);
        $pdf = PDF::loadview('workorder.pdfprint2-template',['data' => $data,'repair'=>$repair,'counter'=>0,'countdb'=>$countdb,'check'=>$checkstr,'countrepairitre' => $countrepairitr])->setPaper('A4','portrait');
   ...
   return $repair1;
    ...
    public function getrepair2(Request $req,$rc2){
        $repair2 = DB::table('rep_mstr')
            ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
            ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
            ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
            ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
    ...
    public function getrepair3(Request $req,$rc3){
        
        $repair3 = DB::table('rep_mstr')
                ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
                ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
                ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
                ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
    ...
    // ->groupBy('spt_code')
        ...
        public function getgroup1(Request $req, $rg1){
            //dd($rg1);
            // $test = DB::table('xxrepgroup_mstr')
            // // ->selectRaw('count(xxrepgroup_rep_code)')
            // ->where('xxrepgroup_mstr.xxrepgroup_nbr','=',$rg1)
            // ->groupBy('xxrepgroup_rep_code')
            // ->get();
            // dd($test);
            $group1 = DB::table('xxrepgroup_mstr')
                ...
                ->join('rep_mstr','xxrepgroup_mstr.xxrepgroup_rep_code','rep_mstr.rep_code')
                ->join('rep_det','rep_mstr.rep_code','rep_det.repdet_code')
                ->join('sp_mstr','rep_det.repdet_part','sp_mstr.spm_code')
                ->join('sp_type','sp_mstr.spm_type','sp_type.spt_code')
                ->join('ins_mstr','rep_det.repdet_ins','ins_mstr.ins_code')
