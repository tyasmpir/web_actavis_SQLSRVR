...
//wo browse
    ...
    public function wocreatedirectmenu(){
        ...
        if (strpos(Session::get('menu_access'), 'WO05') !== false) {
            ...
            $instruction = DB::table('ins_mstr')
                        ->get();
            return view('workorder.wocreatedirect', ['instruction' => $instruction,'data' => $data,'user' => $engineer,'engine'=>$engineer,'asset1'=>$asset,'asset2'=>$asset,'failure' =>$failure,'usernow' =>$usernow,'dept'=>$depart,'fromhome' => '','repairgroup'=>$repairgroup, 'repaircode'=>$repaircode,'sparepart'=>$sparepart]);
        } else {
            toast('Anda tidak memiliki akses menu, Silahkan kontak admin', 'error');
            return back();
        }
    ...
    public function geteditwo(Request $req){
            ...
            $data = DB::table('wo_mstr')
                ...
                wo_finish_time,wo_repair_hour,asset_last_mtc,asset_last_usage_mtc,asset_measure')
        ...
        if($srnbr == null){
            if($req->switch2 =='approve'){
                ...
                if($req->formtype == 1){
                    ...
                    toast('Work Order '.$wonumber.' rejected ', 'success');
                    return redirect()->route('womaint');
                }
            }
        }
