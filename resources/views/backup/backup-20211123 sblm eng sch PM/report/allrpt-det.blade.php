...
<a href="javascript:void(0)" class="viewwo" data-toggle="modal"  title="View WO"  data-target="#viewModal" 
    ...
    @if($ds->wo_status == 'plan')
        <span class="badge badge-primary">{{$ds->asset_desc}} - {{$eng}}</span>
    @elseif($ds->wo_status == 'started')
        <span class="badge badge-warning">{{$ds->asset_desc}} - {{$eng}}</span>
    @elseif($ds->wo_status == 'open')
        <span class="badge badge-danger">{{$ds->asset_desc}} - {{$eng}}</span>
    @elseif($ds->wo_status == 'finish')
        <span class="badge badge-success">{{$ds->asset_desc}} - {{$eng}}</span>
    @elseif($ds->wo_status == 'closed')
        <span class="badge badge-secondary">{{$ds->asset_desc}} - {{$eng}}</span>
    @endif
