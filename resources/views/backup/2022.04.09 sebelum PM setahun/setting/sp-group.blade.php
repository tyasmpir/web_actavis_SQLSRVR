...
@section('content-header')
      <div class="container-fluid">
        <div class="row">          
          <div class="col-sm-4">
            <h1 class="m-0 text-dark">Spare Part Group Maintenance</h1>
          </div>    
        </div><!-- /.row -->
        <div class="col-md-12">
          <hr>
        ...
        <div class="row">                 
          <div class="col-sm-2">    
            <button class="btn btn-block btn-primary" data-toggle="modal" data-target="#createModal">Spare Part Group Create</button>
          </div><!-- /.col -->  
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
@endsection
    ...
    <div class="col-md-4 mb-2 input-group">
        <select id="s_code" class="form-control" name="s_code">
            ...
            @foreach($datasearch as $sdata)
                <option value="{{$sdata->spg_code}}">{{$sdata->spg_code}} - {{$sdata->spg_desc}}</option>
            @endforeach
        </select>
    </div>
...
<div class="table-responsive col-12">
    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
        ...
        <tbody>
            <!-- untuk isi table -->
            @include('setting.table-sp-group')
        </tbody>
    </table>
    <input type="hidden" name="hidden_page" id="hidden_page" value="1"/>
    <input type="hidden" name="hidden_column_name" id="hidden_column_name" value="spg_code"/>
...
<div class="modal fade" id="createModal" role="dialog" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" method="post" action="/createspg">
                {{ csrf_field() }}
...
<div class="modal fade" id="editModal" role="dialog" aria-hidden="true" data-backdrop="static">
    ...
    <div class="modal-content">
        ...
        <form class="form-horizontal" method="post" action="/editspg">
            {{ csrf_field() }}
            <div class="modal-body">
                <div class="form-group row">
...
<div class="modal fade" id="deleteModal" role="dialog" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title text-center" id="exampleModalLabel">Spare Part Group Delete</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <form class="form-horizontal" method="post" action="/deletespg">
                {{ csrf_field() }}
                <div class="modal-body">
                    <input type="hidden" id="d_code" name="d_code">
                    Delete Spare Part Group <b><span id="td_code"></span> -- <span id="td_desc"></span></b> ?
                </div>
...
@section('scripts')
    <script>
       ...
       function fetch_data(page, sort_type, sort_by, code, desc){
            $.ajax({
                url:"spgmaster/pagination?page="+page+"&sorttype="+sort_type+"&sortby="+sort_by+"&code="+code+"&desc="+desc,
       ...
       $(document).on('click', '#btnrefresh', function() {
        ...
        $(document).on('change','#t_code',function(){
            ...
            $.ajax({
              url: "/cekspg?code=" + code + "&desc=" + desc,
              success: function(data) {
                
                if (data == "ada") {
                  alert("Spare Part Group Already Registered!!");
