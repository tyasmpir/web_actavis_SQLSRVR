      ...
      <div class="container-fluid">
        <div class="row">          
          <div class="col-sm-12 col-md-12">
            <h1 class="m-0 text-dark">{{$eng}}'s Work Order</h1>
          </div>    
        </div><!-- /.row -->
        <div class="col-md-12">
          <hr>
...
<input type="hidden" id="woeng" value="{{$eng}}"/>
</div>
...
$(document).on('click', '.pagination a', function(event) {
  ...
  function fetch_data(page, sort_type, sort_by, wonumber,woasset, wostatus,wopriority,woperiod) {
    var urlnow = document.getElementById('woeng').value;
      // asset = urlnow.substring(urlnow.lastIndexOf('/')+1,urlnow.lastIndexOf('('))
      // alert(urlnow);
    $.ajax({
      url: "/woeng/"+urlnow +"/pagination?page=" + page + "&sorttype=" + sort_type + "&sortby=" + sort_by + "&wonumber=" + wonumber + "&woasset=" + woasset + "&wostatus=" + wostatus + "&wopriority="+wopriority+"&woperiod="+woperiod,
