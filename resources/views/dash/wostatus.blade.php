...
<label for="s_nomorwo" class="col-md-2 col-form-label text-md-left">{{ __('Work Order Number') }}</label>
  ...
  <div class="col-md-3 col-sm-12 mb-2 input-group">
    <select id="s_status" type="text" class="form-control"  name="s_status">
      <option value="" {{($vs_status == '') ? 'selected' : ''}}>--Select Status--</option>
      <option value="plan" {{($vs_status == 'plan') ? 'selected' : ''}}>Plan</option>
      <option value="open" {{($vs_status == 'open') ? 'selected' : ''}}>Open</option>
      <option value="started" {{($vs_status == 'started') ? 'selected' : ''}}>Started</option>
...
<script type="text/javascript">
  ...
  function fetch_data(page, sort_type, sort_by, wonumber,woasset, wostatus,wopriority,woperiod) {
    ...
    $.ajax({
      url: "/wostatus/"+urlnow +"/pagination?page=" + page + "&sorttype=" + sort_type + "&sortby=" + sort_by + "&wonumber=" + wonumber + "&woasset=" + woasset + "&wostatus=" + wostatus + "&wopriority="+wopriority+"&woperiod="+woperiod,
